...
$(function() {
	...
	xnCdCombobox({
		id:'accountCompanyClass',
		typeCode:'ACCOUNT_COMPANY_CLASS'
	});
	//是否开启
	xnCdCombobox({
		id:'openOrClose',
...
//			j_grid({
//				toolbar:"#posthd",
//				url:saerchPostUrl+'?deptId='+row.deptId,
//				columnFields:'rowId,deletedFlag,deptId,duty,jobTitle,dutyLvl,qualification',
//				columnTitles:'ID,deletedFlag,deptId,职务,职称,职级,任职资格',
...
function editCompany() {
	...
	var node = $('#treeTable').treegrid('getSelected');
//	var father=null;
//	node.parentId == null ? father=$('#treeTable').treegrid('getRoot'):father=$('#treeTable').treegrid('find',node.parentId);
	//var father = node.parentId == null ?$('#treeTable').treegrid('getRoot'):$('#treeTable').treegrid('find',node.parentId);
	if (node) {
		...
		$('#oldCompanyMark').val(data.companyMark);
...
function addChildCompany() {
	...
	if (node) {
		if(node.isBloc == '0'){
			...
			$('#editFormCompany').form('load',{'cname':node.text,'supCompanyId':node.farmId,'companyMark':node.companyMark});
			$.each($('#isBloc').find('input[type=radio]'),function(index,isBlocData){
				isBlocData.disabled = false;
			});
...
function addDeptChild() {
	...
	if (node) {
		...
		//father=$('#treeTable').treegrid('find',node.farmId);
...
function onBtnSave(str) {
	var formstr = 'editForm' + UpperFirstLetter(str);
	var divstr = 'edit' + UpperFirstLetter(str);
	var btnId = 'btn' + UpperFirstLetter(str) + 'Save';
	var  queryParams = {
		status:1,
		deletedFlag:0
	};
	...
	$('#'+btnId).attr('disabled',true).addClass('btn-disabled');
	$.messager.progress();
	$('#'+formstr).form('submit', {
		url : saveUrl,
		...
		onSubmit : function() {
			...
			if (!isValid) {
				$('#'+btnId).attr('disabled',false).removeClass('btn-disabled');
...
/*//自定义下拉框组件
function MyCdCombobox(pms){	
	
	var url = pms.value == null ? basicUrl+'/param/searchByTypeCode.do?typeCode='+pms.typeCode : basicUrl+'/param/searchByTypeCode.do?typeCode='+pms.typeCode+'&linkValue='+ pms.value;
	$('#'+pms.id).combobox({
		url:url,
	    ...
	    onLoadSuccess: function (data) {
            if (data && data.length > 0) {
            	$.each(data,function(index,codeData){
            		if(codeData.isDefault == 'Y'){
            			$('#'+pms.id).combobox('setValue',codeData.codeValue);
            		}
            	})
            }
        },
		onSelect:function(rec){
			if(pms.linkCode != null && pms.linkField != null){ 
				for(var i=0;i<pms.linkCode.length;i++){
					$('#'+pms.linkField[i]).combobox({
				          valueField:'codeValue', // 值字段
				          textField:'codeName', // 显示的字段
				          url:basicUrl+'/param/searchByTypeCode.do?typeCode='+pms.linkCode[i]+'&linkValue='+rec.codeValue,
				          panelHeight:'auto',
				          required:true,
...
function onSupSure(){
	...
	if(node){
		...
		if(treeTableSelect.type == '公司'){
			...
			$('#companyMark').val(node.companyMark+","+$('#companyRowId').val());
		}else{
...
//猪舍选择确定
function onHouseSure(){
	var nodes = $('#houseTable').datagrid('getSelections');
	if(nodes){
		rightSlipFun('houseSearch',390);
		var houseNames = '',houseIds = '';
		$.each(nodes,function(i,item){
			houseNames += ','+item.houseName;
			houseIds += ','+item.rowId;
		});
		$('#houseNames').searchbox('setText',houseNames);
