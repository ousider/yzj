...
function onRoleTypeChange(newValue,oldValue){
	...
	var templateIdData = jAjax.submit(prUrl+'searchTemplateTypeForRole.do',data[0]);
	$('#templateId').combobox('loadData',templateIdData);
}
//选择人员方法
			...
			if (checked) {
                ...
                if (childNode.length > 0) {
                    for (var i = 0; i < childNode.length; i++) {
                        $("#MenuTree").tree('check', childNode[i].target);
                    }
                }
            } else {
                var childNode = $("#MenuTree").tree('getChildren', node.target);
