...
var disableUrl=prUrl+'editDisablesEmployee.do';//人员停用
var enableUrl=prUrl+'editEnablesEmployee.do';//人员启用
...
$(document).ready(function(){
	...
	$('#table').datagrid(
			j_grid({
				...
				columnFields:'rowId,deletedFlag,employeeCode,employeeName,sex,birthday,nationality,employeeTypeName,postName,entryDate,statusName',
				columnTitles:'ID,deletedFlag,工号,姓名,性别,出生日期,民族,人员类型,岗位,入职日期,启用情况',
				...
				onDblClickRowFun:function(index,row){
					...
					$("#wechatModuleId").combobox('disable');
				}
			})
	);
		...
		onChange:function(newValue,oldValue){
			...
			for (var n = 0; n < newValue.length; n++) {
				...
				for (var x = 0; x < allData.length; x++) {
					// 匹配插入到moduleId选择框
					if(allData[x].rowId == newValue[n]){
						var wechatModuleId = allData[x].wechatModuleId;
						// 排除已经在循环内的resultData中重复的moduleId
						// 跳出外部循环existFlag
						var existFlag = false;
						for(var z = 0; z < wechatResultData.length; z++){
							if(wechatResultData[z].id==wechatModuleId){
								existFlag = true;
								break;
							}
						}
						...
						wechatResultData.push({
							"id":wechatModuleId,
							"text":allData[x].wechatModuleName
						});
						break;
					}
				}
			...
			$("#wechatModuleId").combobox('loadData',wechatResultData);
			$("#wechatModuleId").combobox('clear');
			...
			// 只有一条默认选择第一项
			if($("#wechatModuleId").combobox('getData').length == 1){
				$("#wechatModuleId").combobox('setValue',$("#wechatModuleId").combobox('getData')[0].id);
			}
		}
	});
	...
	xnCombobox({
		...
		onChange:function(newValue,oldValue){
			...
			$("#wechatModuleIdUser").combobox('loadData',wechatResultData);
			$("#wechatModuleIdUser").combobox('clear');
			// 只有一条默认选择第一项
			if($("#moduleIdUser").combobox('getData').length == 1){
				$("#moduleIdUser").combobox('setValue',$("#moduleIdUser").combobox('getData')[0].id);
			}
			// 只有一条默认选择第一项
			if($("#wechatModuleIdUser").combobox('getData').length == 1){
				$("#wechatModuleIdUser").combobox('setValue',$("#wechatModuleIdUser").combobox('getData')[0].id);
			}
		}
	});
...
function updatePas(index){
    ...
    if (row){
    	...
    	if (response.length != 0) {
    			...
    			$('#wechatModuleIdUser').combobox('setValue',data.wechatModuleId);
...
function upOrDownForUserInfo(para){
	if(para.checked){
		...
		$("#wechatModuleId").combobox('enable');
	}else{
		$('#userInfo').css('display','none');
		$("#userName").textbox('disable');
		$("#roleId").combobox('disable');
