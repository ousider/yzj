...
var deleteUrl=prUrl+'deleteCustomesrAndSuppliers.do?cussupType=C';
...
/** *********页面渲染开始*************************************************************************/
//查询按钮
var CLIENTELE_flag = jAjax.submit('/param/getSettingValueByCode.do?settingCode=SC_GROUP_CLIENTELE');
...
//				toolbarList:buttonList,
				...
				columnFields:'rowId,deletedFlag,companyCode,companyName,sortName,createTypeName,catalog,collectmnent,creditLimit,createDate,createrName,changeHistory,companyAddress,notes,alreadyUpdateFarmCount,requestUpdateFarm',
				columnTitles:'ID,deletedFlag,客户编号,客户名称,客户简称,创建类型,累计销售数量,收款条件,信用额度,创建时间,创建人,变更历史,公司地址,备注,同步数量,需要同步的猪场',
				hiddenFields:'rowId,deletedFlag,companyCode',
				columnWidths:'100,100,100,100,100,100,100,100,100,100,100,100,150,100,80,120',
...
//			required:true,
		...
		xnCombobox({
			id:"customerParent",
			url:'/basicInfo/searchHrCompanyClienteleBillToList.do?cussupType=C',
			valueField:"clienteleId",
			textField:"clienteleName",
		});
...
function onBtnView(){
	...
	if(accountListTable!=null){
		$('#accountListTable').datagrid('load',basicUrl+'/basicInfo/searchHistoryAccount.do?cussupId='+record[0].rowId+'&cussupType=C');
	}
	
	if(linkListTable!=null){
		$('#linkListTable').datagrid('load',basicUrl+'/basicInfo/searchHistoryLink.do?cussupId='+record[0].rowId+'&cussupType=C');
	}
}
/**
 * 编辑
...
function onDblClickRow(index,row){
	...
	if(accountListTable!=null){
		$('#accountListTable').datagrid('load',basicUrl+'/basicInfo/searchHistoryAccount.do?cussupId='+row.rowId+'&cussupType=C');
	}
	
	if(linkListTable!=null){
		$('#linkListTable').datagrid('load',basicUrl+'/basicInfo/searchHistoryLink.do?cussupId='+row.rowId+'&cussupType=C');
	}
}
/**
 * 删除
...
function onBtnAdd(){
	...
	if(CLIENTELE_flag == "ON"){
		$('#customerType').combobox({required:true});
		$('#sowSize').textbox({required:true});
		$('#customerLevel').combobox({required:true});
		$('#collectCondition').combobox({required:true});
		$('#collectDays').combobox({required:true});
		$('#creditLimit').textbox({required:true});
		$('#isSaleAccount').combobox({required:true});
	}else{
		$('#customerType').combobox({required:false});
		$('#sowSize').textbox({required:false});
		$('#customerLevel').combobox({required:false});
		$('#collectCondition').combobox({required:false});
		$('#collectDays').combobox({required:false});
		$('#creditLimit').textbox({required:false});
		$('#isSaleAccount').combobox({required:false});
	}

	$('#customerParent').combobox('loadData', jAjax.submit('/basicInfo/searchHrCompanyClienteleBillToList.do?cussupType=C'));
	...
	if(createType == '1'){
		...
		xnComboGrid({
		    ...
		    onChange:function(newValue,oldValue){
		    	...
		    	$('#accountListTable').datagrid('load',basicUrl+'/basicInfo/searchHistoryAccount.do?cussupId='+newValue+'&cussupType=C');
		    	$('#linkListTable').datagrid('load',basicUrl+'/basicInfo/searchHistoryLink.do?cussupId='+newValue+'&cussupType=C');
		    }
		});
