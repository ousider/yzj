...
var saveUrl=prUrl+'toChildCare.do';
var editIndex = undefined;
var eventName = 'TO_CHILD_CARE';
//默认值为3
var oldPigType = 3;
var changeHouseData = [];
...
$.ajax({  
    url : basicUrl+'/param/searchByTypeCode.do?typeCode=CHILD_CARE_CHANGE_TYPE',
    async : false, 
    type : "POST",  
    dataType : "json",  
...
$(document).ready(function(){
	$('#listTable').datagrid(
			...
			columns:[[
		              	...
		              	j_gridText(houseGridComboGrid({
		              		...
		              		url:'/basicInfo/searchHouseToList.do?lineId=0&houseType=8',
		              	...
		              	j_gridText({field:'enterDate',title:'转保育日期',width:150,editor:{type:'datebox',options:{editable:false}}}),
		              	//技术员
		    ...
		    onBeginEdit:function(index,row){
				...
				if(house != null){
					...
					if(row.changeHouseType == 3){
						houseGrid.datagrid('reload',basicUrl+'/basicInfo/searchHouseToList.do?lineId=0&houseType=8&eventName=' + eventName + '&changeType='+row.changeHouseType);
					}else{
						houseGrid.datagrid('reload',basicUrl+'/basicInfo/searchHouseToList.do?lineId=0&houseType=6&eventName=' + eventName + '&changeType='+row.changeHouseType);
					}
				}
				var swinery = $('#listTable').datagrid('getEditor', {
			        index: index,
				...
				if(pigpen != null){
					...
					if(row.changeHouseType == 3){
						...
						pigpenGrid.datagrid('reload',basicUrl+'/basicInfo/searchValidPigpenToList.do?lineId=0&houseType=6&eventName=' + eventName + '&changeType='+row.changeHouseType + '&houseId=' + houseId);
					}
				}
			}
		})
	...
	$('#preSaveRecordTable').datagrid(
			j_grid_view({
				fitColumns:false,
				haveCheckBox:false,
				columnFields:'changeHouseTypeName,mergeSwineryName,houseIdName,pigpenIdName,swineryIdName,pigInfo,pigQty,childQty,childWeight,inHouseIdName,inPigpenIdName,enterDate,workerName,notes',
				columnTitles:'转舍类型,合并群,猪舍名称,猪栏名称,批次名称,猪只信息,猪只数量,转保育数量,重量(KG),转入猪舍,转入猪栏,转保育日期,技术员,备注',
				hiddenFields:'workerName,notes',
				columnWidths:'100,100,100,100,100,350,90,100,100,100,100,100,100,100',
				fit:false,
				width:'auto',
				height:'100%',
				pagination:false
	...
	xnCdCombobox({
		id:'changeHouseType',
		typeCode:'CHILD_CARE_CHANGE_TYPE',
	...
	if(changeHouseDefaultValue == 3){
		houseUrl += '&houseType=8';
	}else{
		houseUrl += '&houseType=6';
	}
	//猪舍
	xnComboGrid({
		id:'houseId',
	...
	$('#searchToChildPigTable').datagrid(
			j_grid({
			width:'auto',
			height:'auto',
...
function searchChangeHouseTypeChange(newValue,oldValue){
	...
	if(newValue == 3){
		...
		houseGrid.datagrid('reload',basicUrl+'/basicInfo/searchHouseToList.do?lineId=0&houseType=6&eventName=' + eventName + '&changeType='+newValue);
	}
}
...
function selectToChildCarePig(){
	leftSilpFun('selectToChildCarePigWin',true);
}
/**
 * 搜索猪只
 */
function searchPigSearch(){
	...
	if(changeHouseType == 3){
		if(pigClassId != '' && pigClassId != 15){
			$.messager.alert('警告', '【保育内转】，猪只状态只能为【保育】状态，请重新选择猪只状态！');
			return;
		}
		pigClassId = 15;
	}else if(changeHouseType == 4){
		if(pigClassId != '' && pigClassId != 14){
			$.messager.alert('警告', '【产房转保育】，猪只状态只能为【断奶】状态，请重新选择猪只状态！');
			return;
		}
		pigClassId = 14;
	}
	$('#searchToChildPigTable').datagrid('load',basicUrl+'/production/searchValidPigToPage.do?queryType='+2+'&choice='+1+'&eventName='+eventName+'&earBrand='+earBrand
...
function selectToChildCarePigSure(){
	var selectRows = $('#searchToChildPigTable').datagrid('getSelections');
