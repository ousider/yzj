...
function onBtnRefreshSYSCache(){
	$.messager.confirm('提示', '确定要刷新缓存吗？', function(r){
		if(r){
			$.post({
				url:basicUrl + '/backEnd/onBtnRefreshSYSCache.do',
				success:function(response){
					response=eval('('+response+')');
					if(response.success){
						$.messager.progress('close');
...
function onBtnRefreshPigCache(){
	$.messager.confirm('提示', '确定要刷新缓存吗？', function(r){
		if(r){
			$.post({
				url:basicUrl + '/backEnd/onBtnRefreshPigCache.do',
				success:function(response){
					response=eval('('+response+')');
					if(response.success){
						$.messager.progress('close');
...
function onBtnRefreshMaterialInfoCache(){
	$.messager.confirm('提示', '确定要刷新缓存吗？', function(r){
		if(r){
			$.post({
				url:basicUrl + '/backEnd/onBtnRefreshMaterialInfoCache.do',
				success:function(response){
					response=eval('('+response+')');
					if(response.success){
						$.messager.progress('close');
