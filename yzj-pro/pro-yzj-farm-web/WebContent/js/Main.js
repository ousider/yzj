...
document.onmousedown = function(e){
	...
	if(e.clientX < 85 || e.clientX > 847 || e.clientY < clientHeight - 508){
...
$(document).ready(function(){
	initUpdateFunction();
	var figureurl_qq_1 = getQueryString('figureurl_qq_1');
	if(figureurl_qq_1){
		$('#personalCenter').replaceWith('<div id="personalCenter" class="login_pic" style="background:url('+figureurl_qq_1+') center no-repeat"></div>');
	}
	var headimgurl = getQueryString('headimgurl');
	if(headimgurl){
		$('#personalCenter').replaceWith('<div id="personalCenter" class="login_pic" style="background:url('+headimgurl.substring(0,headimgurl.length-1)+'46'+') center no-repeat"></div>');
	}
	loadNavMenu();
	...
	$('#changeFarm').css('height',document.documentElement.clientHeight+'px');
	$('#changePassword').find('.rightSlipContent').css('height',document.documentElement.clientHeight - 152 +'px');
	$('#changeFarm').find('.rightSlipContent').css('height',document.documentElement.clientHeight - 152 +'px');
	setLoginMessage();
...
function initUpdateFunction(){
	 jAjax.submit("/param/getUpdateContent.do",null,function(updateContent){
		 if(updateContent.length >= 1){
				var updateLog = updateContent[0].updateLog;
				var updateDate = updateContent[0].updateTime;
				var version = updateContent[0].version;
				var arry = escape(updateLog).split('%0D%0A');
				$("#topDateLog").window('open');
				$('#topDateLogDiv').append('<p>更新时间：'+updateDate+'</p>');
				$('#topDateLogDiv').append('<p>更新版本：'+version+'</p>');
				var logHtml = '<div><p>内容：</p><br>';
				$.each(arry,function(i,item){
					logHtml += unescape(item)+'<br>';
				});
				logHtml += '</div>';
				$('#topDateLogDiv').append(logHtml);
			}
	 },null,null,true);
}
...
function menuClickFun(para){
	...
	if(liLength >= 15){
		...
		if(eventUrl.indexOf('http') != -1){
			...
			if(eventUrl.indexOf('SC') != -1){
				eventUrl +='&companyMark='+loginUser.companyMark;
			}
			//如果是需要从新打开浏览器tab页的地址
			jAjax.submit("/logs/editActionUrl",{url:para.dataset.url,text:para.textContent,id:para.id});
			...
			if(flag){
				...
				if(eventName == 'EventInput'){
					...
					$('#main_right').append('<div id="tabContent_'+eventName+'" style="height:100%"><iframe scrolling="no" frameborder="0" id="iframe_'+eventName+'" src="'+basicUrl+eventUrl+'?randomnumber='+randomnumber+'&fontSize='+baseFontSize+'" style="width:100%;height:100%;"></iframe></div>');
					jAjax.submit("/logs/editActionUrl",{url:eventUrl,text:chinseEventName,id:para.id});
...
function loadNavMenu(){
	...
	loginUser.companyMark = result.companyMark;
	//添加一级菜单
			...
			$.each(data.children,function(childIndex,childData){
				if(childData.children != null){
					...
					$.each(childData.children,function(childChildIndex,childChildData){
						secondAndThirdMenuHtml += '<a href="#" id="'+childChildData.id+'" style="display:inline-block;margin-right:10px;" data-eventName="'+childChildData.component+'" data-url="'+childChildData.url+'" onClick="menuClickFun(this)">'+childChildData.moduleName + '</a>';
					})
					secondAndThirdMenuHtml += '</div></div>';
				}else{
					secondAndThirdMenuHtml += '<div class="secondAndThirdMenu"><div class="secondMenu"><a href="#"  id="'+childData.id+'"  data-eventName="'+childData.component+'" data-url="'+childData.url+'" onClick="menuClickFun(this)">'+childData.moduleName+'</a></div><div>';
				}
			})
		}
...
function loadShortCut(quickMenusList,addedQuickMenus){
	...
	$.each(addedQuickMenus,function(i,addedQuickMenu){
		addedQuickMenusHtml += '<li onClick="menuClickFun(this)" id="'+addedQuickMenu.moduleId+'"'
...
function openFarmWin(){
	openWindowMask({z_index:3});
	$('#changeFarm').animate({right:'0'});
	var farmList = $(".farmList");
	farmList.empty();
	$.ajax({  
	    url : basicUrl+'/basicInfo/searchFarmBind.do',
	    async : false, 
	    type : "POST",  
	    dataType : "json",  
	    success : function(response) { 
	    	var binds = response.rows;
	    	if(binds!=null && binds.length>0){
	    		$.each(binds,function(index,data){
		    		farmList.append('<li><a onclick="switchUser(\''+data['companyCode']+'\',\''+data['userName']+'\',\''+data['password']+'\')" href="javascript:void(0)">'+data['farmName']+'-'+data['userName']+'</a></li>');
	    		});
	    	}
	    }  
	});
}
function switchUser(companyCode,userName,password){
	$.post({
	    url : basicUrl+'/login/login.do',
	    data:{
...
function closeFhangeFarmWin(){
	$('#changeFarm').animate({right:'-390px'});
	$('.window-mask').remove();
}
/**
 * 保存密码
