...
$(document).ready(function(){
	...
	changeRightSlipWinHight();
	if(typeof(eventName) != "undefined" && $('#billDate') != null && eventName != 'GOOD_PIG_SELL'){
		$('#billDate').datebox({
...
function leftSilpFun(id,openMask,z_index){
	...
	var openMaskFlag = openMask == null ? false : openMask;
	if(openMaskFlag){
			...
			if(listTable != null){
				...
				for(var i = 1 ; i < beginY.length ; i ++){
					...
					for(var j = 0 ; j < beginX.length ; j ++){
						...
						var coloptions = listTable.datagrid('getColumnOption',field);
						if(!coloptions.pable){
							$.messager.alert('警告',coloptions.title + '不支持p功能请重新选择！');
							return;
						}else{
							rows[beginY[i]][field] = rows[beginY[0]][field];
							var plinkField = coloptions.plinkField;
							if(plinkField != undefined && plinkField.length > 0){
								for(var link = 0; link < plinkField.length ; link ++){
									rows[beginY[i]][plinkField[link]] = rows[beginY[0]][plinkField[link]];
								}
							}
							if(rows[beginY[0]][field+'Name'] != undefined){
								rows[beginY[i]][field+'Name'] = rows[beginY[0]][field+'Name'];
							}
							if(typeof coloptions.pfunction == 'function'){
								coloptions.pfunction(rows[beginY[i]],rows[beginY[0]]);
							}
						}
					}
				}
				//刷新选中单元格
				refreshChangeDate(beginY);
			}
		}
	        ...
	        if(name == 'BODY'){  
	        	if(window.clipboardData != undefined){
			        ...
			        for (var i = 0; i < textArray.length; i++) {
			        	var newTextArry = textArray[i].split('\t');
			        	$.each(newTextArry,function(n,newText){
			        		//去掉粘贴板上换行特殊字符
			        		newTextArry[n] = escape(newText).replace(/\%0D/g,'');
			        	})
			            array.push(newTextArry);
			        }
...
function returnTarName(pms,row){
	...
	$.each(textArry,function(a,aText){
		$.each(tarRows,function(t,tData){
			if(tData[pms.tarName] == aText){
				i.push(t);
				return false;
			}
		});
	...
	if(i.length > 0){
		...
		for(var j = 0; j < i.length ; j++){
			if(j != i.length - 1){
				newText += tarRows[i[j]][pms.tarName] + ',';
			}else{
				newText += tarRows[i[j]][pms.tarName];
			}
		}
		row[pms.tarFiled + 'Name'] = newText;
	}else{
		...
		for(var i = 0 ; i < beginY.length ; i ++){
			...
			if(i < arry.length){
				for(var j = 0 ; j < beginX.length ; j ++){
					var field = $(cellTdArry[beginX[j]]).attr('field');
					var coloptions = listTable.datagrid('getColumnOption',field);
					if(!coloptions.copyable){
						$.messager.alert('警告',coloptions.title + '不支持copy功能请重新选择！');
						return;
					}else{
						if(j < arry[i].length){
							...
							if(field == 'earBrand' && typeof(eventName) != 'undefined'){
								if(i == 0){
									...
									for(var a = 0 ; a < arry.length ; a++){
										if(arry[a][j].indexOf('\r') > 0){
											arry[a][j]=arry[a][j].substring(0,arry[a][j].indexOf('\r'));
										}
										if(a == arry.length - 1){
											earBrandArry += '\''+arry[a][j]+'\'';
										}else{
											earBrandArry += '\''+arry[a][j]+'\''+',';
										}
									}
									//将jAjax.submit修改成jAjax.submit_dir增加报错信息
									pigData = jAjax.submit_dir('/production/searchValidPigToPage.do',{
										queryType:3,
										query:earBrandArry,
						            	pigType:oldPigType,
						            	specifyPigs:0,
								...
								var pigRow = searchPigByEarBrand(arry[i][j],pigData.rows);
								if(pigRow != null){
									...
									if(eventName == 'WEAN' || eventName == 'CHILD_PIG_DIE' || eventName == 'FOSTER'){
										rows[beginY[i]].pigQty = pigRow.pigQty;
									...
									if(eventName == 'SELECT_BREED_PIG'){
										rows[beginY[i]].pigType = pigRow.pigType;
										rows[beginY[i]].pigTypeName = pigRow.pigTypeName;
										if(rows[beginY[i]].pigType == 1){
											rows[beginY[i]].selectBreedType = 1;
											rows[beginY[i]].selectBreedTypeName = '种公猪';
										}else{
											rows[beginY[i]].selectBreedType = 2;
											rows[beginY[i]].selectBreedTypeName = '种母猪';
										}
									}
									if(eventName == 'SEED_PIG'){
										rows[beginY[i]].houseName = pigRow.houseName;
										rows[beginY[i]].pigpenName = pigRow.pigpenName;
										rows[beginY[i]].pigQty = pigRow.pigQty;
									}
								}
							}
...
function refreshChangeDate(beginY){
	setTimeout(function () {
		for(var i = 0 ; i < beginY.length ; i ++){
			if(typeof(eventName) != 'undefined'){
				...
				if(eventName == 'WEAN'){
					...
					var zeroFlag = false;
					if(weanNum + dieNum> pigQty && !autoFosterDieFlag){
				...
				if(eventName == 'BACKFAT' || eventName == 'CHANGE_LABOR_HOUSE' || eventName == 'WEAN'){
					var row = $('#listTable').datagrid('getRows')[beginY[i]];
					if(row.backfat == ''){
						row.score = '';
					}else if(row.backfat<12){
						row.score = backfatScoreArr.LVL1;
					}else if(row.backfat>=12 && row.backfat<=13){
						row.score = backfatScoreArr.LVL2;
					}else if(row.backfat>=14 && row.backfat<=15){
						row.score = backfatScoreArr.LVL3;
					}else if(row.backfat>=16 && row.backfat<=17){
						row.score = backfatScoreArr.LVL4;
					}else if(row.backfat>=18 && row.backfat<=19){
						row.score = backfatScoreArr.LVL5;
					}else if(row.backfat>=20){
						row.score = backfatScoreArr.LVL6;
					}
				}
				if(eventName == 'SELECT_BREED_PIG'){
					var row = $('#listTable').datagrid('getRows')[beginY[i]];
					if(row.selectBreedType == '1'){
						row.sex = 1;
						row.sexName = '公';
					}else if(row.selectBreedType == '2'){
						row.sex = 2;
						row.sexName = '母';
					}
				}
				if(eventName == 'OUTPUT_USE'){
...
//改变表格显示值
function changeTableDisplayValue(tableId,index,filedAndValueList){
	$.each(filedAndValueList,function(i,item){
		if(item.color != null){
			$("#"+tableId).parent().find('table').find('tr[datagrid-row-index='+index+']').find('td[field="'+item.field+'"]').find('div').css('color',item.color);
		}
		$("#"+tableId).parent().find('table').find('tr[datagrid-row-index='+index+']').find('td[field="'+item.field+'"]').find('div').html(item.value);
	});
}
function focusEditCell(tableId,editIndex,field){
	$('#'+tableId).datagrid('editCell', {index:editIndex,field:field});
	var editor = $('#'+tableId).datagrid('getEditor', {index:editIndex,field:field});
	if(editor != undefined){
		var eidtCellInput = $(editor.target[0].parentNode).find('.textbox-text.validatebox-text');
		eidtCellInput[0].focus();
		eidtCellInput.keydown(function(e){
	    	editCellTabDownFun(e,$('#'+tableId),editIndex,field);
	    });
	}
}
function addDetailEditableList(id){
	var listTable = $('#'+id);
	var defaultValues = new Object(); 
	if(typeof(detailDefaultValues) == "undefined"){
		defaultValues ={
	...
	var index = listTable.edatagrid('getRows').length;
	listTable.edatagrid('addRow',{
		index: index,
		row:defaultValues
	});
}
function deleteDetailEditableList(id){
	var listTable = $('#'+id);
	var record = listTable.datagrid('getSelections');
	var length = record.length;
	if(length < 1){
		...
		if(endEditing(id)){
			$.each(record,function(index,data){
				var rowIndex = listTable.datagrid('getRowIndex',data);
				data.deletedFlag = "1";
				listTable.edatagrid('beginEdit',rowIndex).edatagrid('destroyRow',rowIndex);
			});
		}
	}
}
