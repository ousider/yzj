...
$(document).ready(function(){
	...
	$("#tabContent_homePage").scroll(function() {
	 ...
	 //栏舍周转
	 changePigpenAndHouseByDateType("month");
});
//综合指标
function syntheticalIndicator(dateType){
	jAjax.submit("/portal/searchSyntheticalIndicatorByDate.do?dateType="+dateType,null,function(response){
		$('#nonProductionDays').html('平均'+(response.nonProductionDays).toFixed(2)+'天');
		$('#searchDeliverySurvivalRate').html((response.searchDeliverySurvivalRate).toFixed(2)+'%');
		$('#weanBreedOf7Day').html((response.weanBreedOf7Day).toFixed(2)+'%');
		$('#searchWelfareSurvivalRate').html((response.searchWelfareSurvivalRate).toFixed(2)+'%');
		$('#searchDeliveryProbability').html((response.searchDeliveryProbability).toFixed(2)+'%');
		$('#searchFattenSurvivalRate').html((response.searchFattenSurvivalRate).toFixed(2)+'%');
		$('#searchYearParity').html((response.searchYearParity).toFixed(2)+'胎');
		$('#searchCompleteSurvivalRate').html((response.searchCompleteSurvivalRate).toFixed(2)+'%');
		$('#searchPigBirthWeight').html((response.searchPigBirthWeight).toFixed(2)+'KG');
		$('#search110kgOutHouse').html((response.search110kgOutHouse).toFixed(2)+'天');
		$('#search24DayWeanPigWeight').html((response.search24DayWeanPigWeight).toFixed(2)+'KG');
		$('#searchPSYByDate').html((response.searchPSYByDate).toFixed(2));
		$('#search7030PigWeight').html((response.search7030PigWeight).toFixed(2)+'KG');
		$('#searchMSYByDate').html((response.searchMSYByDate).toFixed(2));
	},null,null,true);
}
//栏舍周转
function changePigpenAndHouseByDateType(dateType){
	var comHtml = '<div class="tabContent-row tabContent-header">'
		+ '<div class="wd-com-after wd-6"><strong>猪栏名称</strong></div>'
		+ '<div class="wd-com-after wd-4"><strong>猪栏状态</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>批次</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>期初</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>出生</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>转入</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>转出</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>死亡</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>存栏</strong></div>'
		+ '</div>';
	$('#tabContent_header').html(comHtml);
	jAjax.submit("/portal/searchChangePigpenAndHouseByDateType.do?dateType="+dateType,null,function(response){
		changePigpenAndHouseAddDate(response.lactationPig,'brPigHouseList');
		changePigpenAndHouseAddDate(response.nursingPig,'byPigHouseList')
		changePigpenAndHouseAddDate(response.fatteningPig,'yfPigHouseList')
		changePigpenAndHouseAddDate(response.productionPig,'szPigHouseList')
	},null,null,true);
}
function displayByPigType(para,type){
	$.each($('#changeHouseAndPigpenUl').find('li'),function(i,li){
		$(li).removeClass('selected');
	});
	$(para).addClass('selected');
	if(type == 'brPig'){
		$('#brPigHouseList').css('display','block');
	}else{
		$('#brPigHouseList').css('display','none');
	}
	if(type == 'byPig'){
		$('#byPigHouseList').css('display','block');
	}else{
		$('#byPigHouseList').css('display','none');
	}
	if(type == 'yfPig'){
		$('#yfPigHouseList').css('display','block');
	}else{
		$('#yfPigHouseList').css('display','none');
	}
	if(type == 'szPig'){
		$('#szPigHouseList').css('display','block');
	}else{
		$('#szPigHouseList').css('display','none');
	}
}
function houseAndPigpenChange(para){
	changePigpenAndHouseByDateType($(para).val());
}

function changePigpenAndHouseAddDate(data,divId){
	$('#'+divId).empty();
	var html = '';
	var sinweyNum = 0,startNum = 0,birthNum = 0,intoNum = 0,outNum = 0,dieNum = 0,nowNum = 0;
	$.each(data,function(i,item){
		sinweyNum += item.SWINERY_NUM;
		startNum += item.START_NUM;
		birthNum += item.BITH_NUM;
		intoNum += item.INTO_NUM;
		outNum += item.OUT_NUM;
		dieNum += item.DIE_NUM;
		nowNum += item.NOW_NUM;
		html += '<div class="tabContent-row">'
			+ '<div class="wd-com-after wd-6">'+item.HOUSE_NAME+'</div>'
			+ '<div class="wd-com-after wd-4">'+item.HOUSE_CLASS+'</div>'
			+ '<div class="wd-com-after wd-2">'+item.SWINERY_NUM+'</div>'	
			+ '<div class="wd-com-after wd-2">'+item.START_NUM+'</div>'
			+ '<div class="wd-com-after wd-2">'+item.BITH_NUM+'</div>'
			+ '<div class="wd-com-after wd-2">'+item.INTO_NUM+'</div>'
			+ '<div class="wd-com-after wd-2">'+item.OUT_NUM+'</div>'
			+ '<div class="wd-com-after wd-2">'+item.DIE_NUM+'</div>'				
			+ '<div class="wd-com-after wd-2">'+item.NOW_NUM+'</div>'				
			+ '</div>'				
	});
	html += '<div class="tabContent-row">'
		+ '<div class="wd-com-after wd-10"><strong>合计</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>'+sinweyNum+'</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>'+startNum+'</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>'+birthNum+'</strong></div>'
		+ '<div class="wd-com-after wd-2"><strong>'+intoNum+'</strong></div>'					
		+ '<div class="wd-com-after wd-2"><strong>'+outNum+'</strong></div>'					
		+ '<div class="wd-com-after wd-2"><strong>'+dieNum+'</strong></div>'	
		+ '<div class="wd-com-after wd-2"><strong>'+nowNum+'</strong></div>'
		+ '</div>'
	$('#'+divId).append(html);
}
...
function syntheticalIndicatorChange(para){
	syntheticalIndicator($(para).val());
}
...
function productionWarnAddData(){
	...
	//种猪淘汰提醒
	$('#obsoleteWarn').empty();
	jAjax.submit("/portal/productionAbnormalByObsolete.do",null,function(response){
		$.each(response,function(i,data){
			if(data != null){
				var html = '<tr>'
					+ '<td class="wd-3">'+data.SORT_NBR+'</td>'
					+ '<td class="wd-16"><a class="fineReport-a" href="#" onClick=openWarnReport("OBSOLETE",'+data.SORT_NBR+')>'+data.MESSAGE+'</a></td>'
					+ '<td class="wd-5 font-red">'+data.PIG_QTY+'</td>'
					+ '</tr>';
				$('#obsoleteWarn').append(html);
			}
		})
	},null,null,true);
}
function productionParamAddData(){
	...
	//配种头数
	 ...
	 //年更母猪
	// var yearUpdateSowValue = baseSow_SV * updateRate_SV;
	 var yearUpdateSowValue = baseSow_SV * updateRate_SV / preSowSucRate_SV;
...
function openWarnReport(type,sortNum){
	...
	var reportUrl = 'http://www.xnplus.cn:4380/WebReport/ReportServer?reportlet=XNPLUS%2Fabnormal.cpt&dbUrl='
...
function reloadAll(){
	 ...
	 productionParamAddData();
	 //存栏规模初始化
	 $('#CLGM_searchRange').val('week');
	 handScaleInit('week');
	 //生产分析初始化
	 $('#SCFX_searchRange').val('week');
	 productionAnalyzeInit('week');
	 //死亡分析初始化
	 $('#SWFX_searchRange').val('week');
	 deathAnalyzeInit('week');
	 //胎次分布初始化
	 parityDescInit(basicSowNum);
	 //母猪群蓝图数据绑定
	 ...
	 productionWarnAddData();
	 //综合指标
	 $('#ZHZB_searchRange').val('year');
	 syntheticalIndicator("year");
	 //栏舍周转
	 $('#LSZZ__searchRange').val('month');
	 changePigpenAndHouseByDateType('month');
}