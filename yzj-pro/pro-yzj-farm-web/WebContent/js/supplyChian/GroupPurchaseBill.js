...
var searchMainListUrl = prUrl + 'searchGroupPurchaseStoreToPage.do';
...
$(document).ready(function(){
	...
	$('#table').datagrid(
			j_grid({
				...
				hiddenFields:'rowId',
				sortFields:'billCode,billDate,eventCodeName,statusName,purchaserName,farmNames,totalPriceName,supplierSortName,notes',
	...
	setTimeout(function(){
		...
		$('#purchaseBillDetailTable').datagrid(
				j_grid_view({
					haveCheckBox:false,
					columnFields:'requireFarmSortName,materialName,materialSecondClassName,specAll,actualPriceName,purchaseQtyName,actualFreePercent,totalPriceName,purchaseOrFree,arriveQtyName,arrivePurchasePercent,notes',
					columnTitles:'需求公司,采购物料,中类,规格,单价,采购数量,赠送比率,总价,购料/赠料,到库数量,入库率,备注',
					hiddenFields:'',
					fit:false,
					pagination:false,
					height:'100%',
...
function changeSupplierIdUrl(checkboxItem){
	var checkedFlag = $(checkboxItem).is(':checked');
	var data = [];
	if(checkedFlag){
		data = jAjax.submit('/supplyChian/searchCompanyFromRequireBillToList.do?searchType=All&purchaseFlag='+purchaseFlag+'&canPurchaseSupplierId='+canPurchaseSupplierId);
	}else{
		data = jAjax.submit('/supplyChian/searchCompanyFromRequireBillToList.do?searchType=Require&purchaseFlag='+purchaseFlag+'&canPurchaseSupplierId='+canPurchaseSupplierId);
	}
	...
	$('#listTable').datagrid(
			j_detailGrid({
				...
				columns:[[
			      	...
			      	j_gridText({field:'requireFarm',title:'需求公司',width:50,
			      		formatter:function(value,row){
		              		return row.requireFarmSortName;
		              	},
			      		editor:xnGridComboBox({
			      			field:'requireFarm',
			      			valueField:'farmId',
			      			textField:'farmSortName',
			      			onChange:requireFarmChange
			      		})
			      	}),
	              	j_gridText({field:'materialName',title:'物料',width:150}),
	              	j_gridText({field:'materialFirstClassName',title:'大类',width:50}),
	              	j_gridText({field:'materialSecondClassName',title:'中类',width:50}),
	              	j_gridText({field:'specAll',title:'规格',width:100}),
	              	j_gridText({field:'requireQty',title:'需求量',formatter:function(value,row){
	              	...
	              	j_gridNumber({field:'actualPrice',title:'单价',formatter:function(value,row){
              			value = value == null || value == undefined || value == '' ? 0 : value;
	              		return value+'元';
	              	},min:0,precision:3,onChange:priceChangeFun,width:70}),
	              	j_gridNumber({field:'purchaseQty',title:'采购数量',formatter:function(value,row){
              			value = value == null || value == undefined || value == '' ? 0 : value;
              			return value+row.unit;
	              	},min:0,precision:3,onChange:purchaseQtyChangeFun,width:70}),
	              	j_gridText({field:'actualFreePercent',title:'赠送比率',width:60}),
	              	j_gridText({field:'totalPrice',title:'总价',formatter:function(value,row){
              			value = value == null || value == undefined || value == '' ? 0 : value;
              			row.totalPrice = value
	              		return value+'元';
	              	},width:80}),
		          	j_gridText({field:'rebateReason',title:'0元原因',width:80,
	              		formatter:function(value,row){
	              			return row.rebateReasonName;
	              		},
	              		editor:
	              		xnGridCdComboBox({
	              			field:'rebateReason',
	              			typeCode:'REBATE_REASON',
	              			editable:false
	              		})
	              	}),
	              	j_gridNumber({field:'rebatePrice',title:'返利金额',formatter:function(value,row){
              			value = value == null || value == undefined || value == '' ? 0 : value;
	              		return value+'元';
	              	},min:0,precision:3,width:80}),
	              	j_gridText({field:'notes',title:'备注',editor:{type:'textbox'},width:70})
			    ]],
			    onBeginEdit:function(index,row){
			    	...
			    	if(requireFarm != null){
			    		if(row.rowIds != undefined && row.rowIds != '' && row.rowIds != null){
			    			var requireFarmData = jAjax.submit('/supplyChian/searchCompanyByFarmIdToList.do');
			    			$(requireFarm.target).combobox('loadData',requireFarmData);
			    			$(requireFarm.target).combobox('disable');
			    			$(requireFarm.target).combobox('setText',row.requireFarmSortName);
			    		}else{
			    			var requireFarmData = jAjax.submit('/supplyChian/searchCompanyByFarmIdToList.do?materialId='+row.relatedId);
			    			$(requireFarm.target).combobox('loadData',requireFarmData);
			    		}
					}
			    	if(row.notBillIds){
			    	...
			    	var rebateReason = $('#listTable').datagrid('getEditor', {
				        index: index,
				        field: 'rebateReason'
				    });
			    	var rebatePrice = $('#listTable').datagrid('getEditor', {
				        index: index,
				        field: 'rebatePrice'
				    });
		    		// 物料采购
			    	if($('#eventCode').combobox('getValue') == 'MATERIAL_PURCHASE'){
			    		// 价格为0
				    	if(!row.actualPrice || strToFloat(row.actualPrice) == 0){
				    		// 0元原因，返利金额可填
				    		if(rebateReason != null){
				    			$(rebateReason.target).combogrid('enable');
				    		}
				    		if(rebatePrice!=null){
				    			$(rebatePrice.target).numberspinner('enable');
				    		}
				    	}else{
					    	if(rebateReason != null){
					    		$(rebateReason.target).combogrid('disable');
					    	}
				    		if(rebatePrice!=null){
				    			$(rebatePrice.target).numberspinner('disable');
				    		}
				    	}
			    	}else{
			    		if(rebateReason != null){
			    ...
			    onEndEdit:function(index,row){
			    	...
			    	if(requireFarm != null){
			    		if(row.requireFarm){
				    		var result = jAjax.submit('/supplyChian/searchTurnMaterialToFarmMaterialToList.do?farmId='+row.requireFarm+'&materialId='+row.materialId)[0];
				    		row.actualFreePercent = result.freePercent;
				    		row.groupId = result.groupId;
				    		row.manufacturer = result.manufacturer;
				    		row.materialId = result.materialId;
				    		row.materialName = result.materialName;
				    		row.materialFirstClass = result.materialFirstClass;
				    		row.materialFirstClassName = result.materialFirstClassName;
				    		row.materialSecondClass = result.materialSecondClass;
				    		row.materialSecondClassName = result.materialSecondClassName;
				    		row.outputMinQty = result.outputMinQty;
				    		row.actualPrice = result.price;
				    		row.spec = result.spec;
				    		row.specAll = result.specAll;
				    		row.specNum = result.specNum;
				    		row.supplierId = result.supplierId;
				    		row.supplierSortName = result.supplierSortName;
				    		row.supplierName = result.supplierName;
				    		row.unit = result.unit;
				    		priceChangeFun(strToFloat(row.actualPrice),0);
			    		}
			    	...
			    	if($('#eventCode').combobox('getValue') == 'MATERIAL_PURCHASE'){
			    		// 价格不为0
				    	if(!(!row.actualPrice || strToFloat(row.actualPrice) == 0)){
				    		row.rebateReason = null;
				    		row.rebateReasonName = null;
				    		row.rebatePrice = null;
				    	}
			    	}else{
			    		row.rebateReason = null;
			    		row.rebateReasonName = null;
			    	...
			    	if(rebateReason != null){
			    		row.rebateReasonName = $(rebateReason.target).combogrid('getText');
			    	}
			    },
			    view: detailview,
			    detailFormatter:function(index,row){
			    	if(row.hasOwnProperty('requireFarm') && row.requireFarm){
				    	if(!row.hasOwnProperty('freeInfo') || row.freeInfo.length==0){
					    	var fixedFreeData = jAjax.submit('/supplyChian/searchFixedFreeMaterialToList.do?relatedId='+row.relatedId+'&requireFarm='+row.requireFarm+'&onlyFree=true&mustBeFree=Y');
			    	...
			    	if(!$('#freeDiv-'+row.nodeIndex).hasClass('hasChange')){
	                    ...
	                    '<tr><td><button type="button" onclick="addFree('+nodeIndex+','+index+')" class="listTableToolbarBtn-detail">新增</button>'+
         			    ...
         			    '<tr class="detail-row"><td style="display:none;">赠送ID</td><td>赠送物料</td><td>赠送规格</td><td>赠送数量</td><td>备注</td></tr>';
...
function initInputingPurchaseBillListTable(){
	...
	$('#listTable').datagrid(
			j_detailGrid({
				...
				columns:[[
			      	{field:'ck',checkbox:true},
			      	j_gridText({field:'requireFarmSortName',title:'需求公司',width:100,sortable:false}),
...
//选择需求物料
function selectRequireMaterial(){
	...
	if(supplierId){
		$('#selectRequiredMaterialListTable').datagrid('loadData',{success:true,total: 0, rows: [] });
		$('#selectRequiredMaterialListTable').datagrid('load','');
		...
		$('#selectRequiredMaterialListTable').datagrid('load',basicUrl+searchSelectRequireMaterialListTableUrl+'?supplierId='+supplierId+'&eventCode='+$('#eventCode').combobox('getValue'));
		leftSilpFun('selectRequiredMaterialWin',true,9001);
	}else{
...
function onBtnAddSelf(){
	...
	$('#supplierId').combobox('loadData', jAjax.submit('/supplyChian/searchCompanyFromRequireBillToList.do?searchType=Require&purchaseFlag='+purchaseFlag+'&canPurchaseSupplierId='+canPurchaseSupplierId));
...
function addFree(nodeIndex,index){
	...
	var freeMaterialUrl = '/supplyChian/searchFixedFreeMaterialToList.do?relatedId='+row.relatedId+'&requireFarm='+row.requireFarm+'&onlyFree=true&mustBeFree=N';
	//赠送物料
	xnComboGrid({
		id:'freeMaterialId',
		idField:'rowId',
...
function onBtnSave(){
	...
	if(saveFlag=='insert'){
		saveUrl = prUrl+'editGroupPurchaseStore.do?editType=insert';
	}else if(saveFlag=='update'){
		saveUrl = prUrl+'editGroupPurchaseStore.do?editType=update';
