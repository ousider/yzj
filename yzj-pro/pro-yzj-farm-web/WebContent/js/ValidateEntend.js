...
$.extend($.fn.validatebox.defaults.rules, {
	grid_combobox_validateExist: {
        validator: function(value, param){
        	var flag = false;
        	var editor = $('#'+param[2]).datagrid('getEditor', {
                index: editIndex,
                field: param[0]
            });
        	if(editor != null){
...
$.extend($.fn.validatebox.defaults.rules, {
	numAddPercent: {
		 validator: function(value){
	        ...
	        message: '输入百分比或者分数，例如22%，2/3 ，1:1（大赠小）'
    }
});
/**
 * 正整数
...
$.extend($.fn.datebox.defaults.rules,{  
    ltCurrentMonthDate: {  
        validator: function(value,param){  
            if(value) {
            	var newDate = new Date(value),currentDate = new Date();
            	var newMonth = newDate.getMonth(),newYear = newDate.getYear(),currentMonth = currentDate.getMonth(),currentYear = currentDate.getYear();
            	if((newMonth == currentMonth || newMonth ==currentMonth - 1) && (newYear == currentYear) && (newDate.getTime() <= currentDate.getTime())){
            		return true;
            	}
            }
            return false;  
        },  
        message:"日期必须选择正确月份日期"  
    }  
}); 
/**
 * 验证只能输入年-周
...
$.extend($.fn.validatebox.defaults.rules, {
	yearWeek: {
		 validator: function(value,param){
	        	...
	        	if(reg.test(value)){
	        		var arr = value.split("-");
	        		var year = param[0],week = param[1];
	        		if(arr[0]<=param[0] && arr[1]<=param[1]){
	        			flag = true;
	        		}
	        	}
	            return flag;
		    },
	        message: '请输入正确周次，如2017-09,且不能大于当前周次'
    }
});
/**
 * 验证数字输入框最小值(不等于)
